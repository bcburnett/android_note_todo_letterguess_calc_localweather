...
class CalcViewModel (application: Application) : AndroidViewModel(application) {
    ...
    companion object {
        @Volatile
        private var INSTANCE: CalcViewModel? = null
        fun newInstance(application: Application): CalcViewModel {
            if (INSTANCE != null) return INSTANCE as CalcViewModel
            synchronized(this) {
                INSTANCE = CalcViewModel(application)
                return INSTANCE as CalcViewModel
            }
        }
    }
